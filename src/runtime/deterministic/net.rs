...
impl InMemoryNetwork {
    ...
    pub async fn connect<A: Into<net::SocketAddr>>(
        ...
        loop {
            if let Some(mut inner) = self.inner.try_lock() {
                ...
                let tcp_stream =
                    InMemoryTcpStream::new_client(client_socket, addr.clone(), addr.clone());
    ...
    pub(crate) async fn bind<A>(&self, addr: A) -> Result<InMemoryListener, io::Error>
    where
        A: Into<net::SocketAddr>,
    {
        loop {
            if let Some(mut inner) = self.inner.try_lock() {
                ...
                let port = addr.port();
                let ch = inner.register_listener_channel(port, tx.clone())?;
