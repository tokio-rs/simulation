...
use std::{collections, io, net, num, pin::Pin, sync, task::Context, time::Duration};
use tokio_executor::park::Park;
mod fault;
mod handle;
mod socket;
use async_trait::async_trait;
use fault::FaultyTcpStream;
use handle::Inner;
...
impl<P> Network<P> {
    pub fn handle(&self, ipaddr: net::IpAddr) -> Result<NetworkHandle, io::Error> {
        // TODO: Throw error and register new handle IP
        let inner = sync::Arc::clone(&self.inner);
        Ok(NetworkHandle::new(ipaddr, inner))
    }
}
...
impl Network<FaultyTcpStream<SocketHalf>> {
    fn new_with_fault() {}
}

pub struct Listener {
    /// Channel containing incoming connections.
    accept: mpsc::Receiver<SocketHalf>,
    local_addr: net::SocketAddr,
}

impl Listener {
    fn new(bind_addr: net::SocketAddr, rx: mpsc::Receiver<SocketHalf>) -> Self {
        Self {
            accept: rx,
...
mod tests {
    ...
    fn bind_and_connect() {
        ...
        let network_handle = network.handle("10.0.0.1".parse().unwrap()).unwrap();
        runtime.block_on(async {
            // spawn server which binds to a port.
            handle.spawn(server(8080, network_handle.clone(), handle.clone()));
            let server_addr = net::SocketAddr::new(network_handle.ip_addr(), 8080);
            let stream;
            loop {
                if let Ok(conn) = network_handle.connect(server_addr).await {
                    stream = conn;
                    break;
                } else {
